...
function groupToRow(group) {
  ...
  const row = {
    ...
    balance: parseCurrency(joinColumn(columns.balance)),
  ...
  if (!/^\d{2}$/.test(row.month) || !/^\d{2}$/.test(row.day) || row.balance === "") {
    return null;
  }
...
function toCsv(rows) {
  const header = ["month", "day", "type1", "station1", "type2", "station2", "balance", "amount", "raw"];
